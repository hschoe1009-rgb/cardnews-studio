...
async function api(path, options = {}) {
  // 웹앱으로 배포된 경우 서버가 없다. webapi.js 가 브라우저 안에서 같은 일을 한다.
  if (window.__WEBAPI__) return window.__WEBAPI__.route(path, options);
...
/* ── 이미지 패널 */
/* 서버가 준 경로면 캐시를 새로 받게 하고, data URL 이면 그대로 둔다.
   data URL 뒤에 ?t= 를 붙이면 base64 가 깨져 PNG 내보내기가 실패한다. */
function freshImage(url) {
  return String(url).startsWith('data:') ? url : url + '?t=' + Date.now();
}
...
$('btn-gen-image').addEventListener('click', async () => {
  ...
  card().image = freshImage(res.image);
  renderStage();
  renderFilmstrip();
});
...
$('btn-download').addEventListener('click', async () => {
  const path = `/api/projects/${state.project.id}/download`;
  if (window.__WEBAPI__) { await run('ZIP 만들기', () => window.__WEBAPI__.download(path)); return; }
...
$('btn-open-db').addEventListener('click', async () => {
  if (window.__WEBAPI__) { await run('엑셀 내려받기', () => window.__WEBAPI__.download('/api/db')); return; }
  window.location = '/api/db';
});
